...
function processFlatsCollection(flats){
  ...
  $.each(flats["flats"], function(i,flats){
      ...
      $("#stings-list").append(listItemHTML(flats.links["self"].uri, flats.address, flats.description, flats.lastModified, flats.creationTimestamp, flats.id, edit, flats.numpartner));
      if(i==0)
...
function listItemHTML(uri, address, description,lastModifield, creationTimestamp, id, edit, numpartner){
