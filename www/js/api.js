...
function createuser(loginid, password, fullname, email, phone, complete){
	loadAPI(function(){
		var api = JSON.parse(sessionStorage.api);
		var uri = "http://147.83.7.207:8888/apartmentshare/users";
		$.post(uri,
			{
				loginid: loginid,
				password: password,
...
function enviarMensaje(text, complete){
	var authToken = JSON.parse(sessionStorage["auth-token"]);
	console.log(authToken.token);
...
function enviarImagen( complete){
	...
	//var imgElem= document.getElementById("formuploadajax");;

var formData = new FormData($('formuploadajax')[0]);
	
	var formData = JSON.stringify(getBase64Image(formData));
    ...
    }).done(function(imgElem) { 
		$("#mensaje").html(imgElem);
    	complete();
  	}).fail(function(jqXHR, textStatus, errorThrown){	
			var error = jqXHR.responseJSON;
				alert(error.reason);
...
function getfotos(uri, complete){
