...
var uri = JSON.parse(sessionStorage["uri-rooms2"]);
   getRooms(uri, function(flats){
      ...
      $("#stings-list").append(listItemHTML(flats.links["self"].uri, flats.address, flats.description, flats.lastModified, 
        flats.creationTimestamp, flats.id, flats.girlorboy, flats.sqm, flats.price, flats.status, flats.userid, flats.furnished));
   });
});
...
function listItemHTML(uri, address, description,lastModified, creationTimestamp, id, girlorboy, sqm, price, status, userid, furnished){
...
var creationTimestamp = new Date( creationTimestampformat );
  ...
  var userid= '<h6 class="list-group-item-heading unclickable" align="center">'+ 'Anuncio creado por el usuario: '+  userid +'</h6>';;
  ...
  return p + g + sqm + furnished + price+ status + userid + l +h ;
}
