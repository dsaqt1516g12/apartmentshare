...
public interface RoomDAOQuery {
    ...
    public final static String GET_ROOMS = "select hex(id) as id, hex(userid) as userid, hex(flatid) as flatid, description, girlorboy,  sqm, furnished,status, price, creation_timestamp, last_modified from room where creation_timestamp < ? order by creation_timestamp desc limit 5";
    public final static String GET_ROOMS_AFTER = "select hex(id) as id, hex(userid) as userid, hex(flatid) as flatid, description, girlorboy,  sqm, furnished, creation_timestamp, last_modified from room where creation_timestamp > ? order by creation_timestamp desc limit 5";
