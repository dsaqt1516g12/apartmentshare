...
public interface FlatDAOQuery {
    ...
    public final static String GET_FLATS = "select hex(id) as id, hex(userid) as userid, hex(campusid) as campusid, address, description, numpartner,  smoker,  pets,  girlorboy,  sqm, creation_timestamp, last_modified from flat where creation_timestamp < ? and userid=unhex(?) order by creation_timestamp desc limit 5 ";
